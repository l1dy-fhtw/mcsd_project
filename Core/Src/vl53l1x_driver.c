...
/* Official ST default configuration parameter block (91 bytes starting at 0x002D) */
...
VL53L1X_Status_t VL53L1X_Init(I2C_HandleTypeDef *hi2c) {
    ...
    /* 1. Hardware Communication Verification */
    ...
    /* 2. Wait for Device Boot Completion */
    while (bootState == 0) {
    ...
    /* 3. Load full DSP / SPAD configuration table (91 bytes starting at 0x002D) */
...
uint8_t VL53L1X_IsDataReady(I2C_HandleTypeDef *hi2c) {
    ...
    if (I2C_ReadReg8(hi2c, VL53L1X_REG_INT_STATUS, &status) == HAL_OK) {
        /* Bit 0 is asserted HIGH when distance calculation is complete */
        if ((status & 0x01) != 0) {
            return 1;
        }
    }
...
/* Official ST VL53L1X status decoding table */
...
VL53L1X_Status_t VL53L1X_GetDistance(I2C_HandleTypeDef *hi2c, uint16_t *pDistanceMm) {
    ...
    *pDistanceMm = rawDistance;
    ...
    /* Status 0 = Valid, 1 = Sigma warning, 2 = Signal warning (still valid data) */
